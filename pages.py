...
LANG = {
  ...
  "fa": {
    ...
    "background_style": "استایل پس‌زمینه",
    "default_blue": "آبی پیش‌فرض",
    "pure_black": "مشکی خالص",
    "dark_grey": "خاکستری تیره",
...
.proto-btn-v .proto-icon.trojan{background:rgba(255,68,68,0.2);color:#ff4444}
...
.proto-btn-v .proto-badge{font-size:8px;background:rgba(255,255,255,0.06);padding:2px 8px;border-radius:4px;color:var(--t3);font-weight:400;margin-left:auto;white-space:nowrap}
[data-theme^="light"] .proto-btn-v{background:rgba(255,255,255,0.4)}
[data-theme^="light"] .proto-btn-v.active{background:var(--accent-d);border-color:var(--accent);color:var(--accent)}
...
.pc-trojan{background:rgba(255,68,68,0.15);color:#ff6666}
        ...
        <div class="proto-group-v" id="proto-group-v">
          <button class="proto-btn-v active" data-proto="vless-ws" onclick="toggleProtoBtnV(this)">
            ...
            <span class="proto-info">
              <span class="proto-name">VLESS · WS</span>
          ...
          <button class="proto-btn-v" data-proto="xhttp-packet-up" onclick="toggleProtoBtnV(this)">
            ...
            <span class="proto-info">
              <span class="proto-name">XHTTP · packet-up</span>
              <span class="proto-desc">Siz10 · Packet Mode</span>
            </span>
            <span class="proto-badge">Ultra</span>
          </button>
          <button class="proto-btn-v" data-proto="xhttp-stream-up" onclick="toggleProtoBtnV(this)">
            <span class="proto-icon xhttp"><i class="ti ti-arrow-wave-right-up"></i></span>
            <span class="proto-info">
              <span class="proto-name">XHTTP · stream-up</span>
              <span class="proto-desc">Siz10 · Stream Mode</span>
            </span>
            <span class="proto-badge">Ultra</span>
          </button>
...
<section class="pg" id="pg-security">
  ...
  <div class="g2">
    <div class="card"><div class="card-title"><i class="ti ti-lock"></i> <span data-lang="encryption">Encryption</span></div>
      ...
      <div class="sr"><span class="sr-k"><i class="ti ti-network"></i> <span data-lang="protocols">Protocols</span></span><span class="sr-v">VLESS · WS, XHTTP · packet, XHTTP · stream, Trojan · WS</span></div>
...
<section class="pg" id="pg-settings">
  ...
  <div class="g2">
    ...
    <!-- ===== Protocol Custom Server Settings (با Trojan) ===== -->
    <div class="card" style="grid-column:1/-1">
      <div class="card-title"><i class="ti ti-server-2"></i> <span>Protocol Custom Names & Templates</span></div>
      <div style="display:flex;flex-direction:column;gap:8px">
        <div class="cl" style="margin-bottom:4px"><i class="ti ti-info-circle"></i> Each protocol can have its own <strong>Server Name</strong>, <strong>Link Prefix</strong> and <strong>Link Template</strong>. Leave empty to use default values.</div>
        <div style="display:grid;grid-template-columns:1.2fr 1.2fr 1.2fr 2fr;gap:8px;font-size:10px;font-weight:700;color:var(--t2);padding:6px 4px;border-bottom:1px solid var(--card-b)">
          <div>Protocol</div>
          <div>Server Name</div>
          <div>Link Prefix</div>
        ...
        <!-- Trojan -->
        <div style="display:grid;grid-template-columns:1.2fr 1.2fr 1.2fr 2fr;gap:8px;align-items:center;background:rgba(255,68,68,0.08);border-radius:8px;padding:6px 4px">
          <div style="font-size:11px;color:#ff6666">Trojan</div>
...
function setBackgroundStyle(style){
  ...
  localStorage.setItem('CBeeNet-bg-style', style);
  applyBackgroundStyle(style);
  document.querySelectorAll('#bg-styles .btn').forEach(btn => {
    btn.style.outline = btn.dataset.bg === style ? '2px solid var(--accent)' : 'none';
    btn.style.outlineOffset = btn.dataset.bg === style ? '2px' : '0';
  });
  toast('Background style changed', 'ok');
}
...
// ========== PROTOCOL CUSTOM SETTINGS (با Trojan) ==========
async function loadProtocolSettings(){
  try {
    const r = await authF('/api/settings/protocol');
    const data = await r.json();
    const protos = ['vless-ws','xhttp-packet-up','xhttp-stream-up','trojan-ws'];
...
async function saveProtocolSettings(){
  ...
  for (const proto of protos) {
    ...
    payload[proto] = {
      ...
      link_template: tmplEl ? tmplEl.value.trim() : ''
    };
  }
  try {
    const r = await authF('/api/settings/protocol', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ protocols: payload })
    });
    const result = await r.json();
    if(result.ok){
      document.getElementById('protocol-save-result').style.display = 'block';
      setTimeout(() => document.getElementById('protocol-save-result').style.display = 'none', 3000);
      toast('Protocol settings saved ✓', 'ok');
      loadLinks();
    } else {
      toast('Error saving protocol settings', 'err');
    }
...
async function fetchStats(){
  try{
    ...
    // اگر interpolation انجام نشد، فقط برای traffic یک نقطه اضافه می‌کنیم (نه دلتا اگر interpolated=true)
    if (!interpolated) {
      // برای ترافیک، اگر delta > 0 و gapSeconds کوچک بود، یک نقطه با دلتا اضافه می‌کنیم
      // اما اگر delta == 0، نقطه صفر اضافه می‌کنیم
      if (delta >= 0) {
      ...
      // اگر interpolation انجام شده، فقط مقدار نمایشی را به‌روز می‌کنیم
      updateChartValue('traffic');
    }
    prevTraf = totalTrafficDisplay;
    localStorage.setItem(PREV_TRAF_KEY, String(prevTraf));

    // برای load و conns همیشه یک نقطه اضافه می‌کنیم
...
function initSecondaryCharts(){
  ...
  dashProtoChart = new Chart(ctxProto, {
    type: 'bar',
    data: { 
      labels: ['VLESS · WS', 'XHTTP · packet', 'XHTTP · stream', 'Trojan · WS'], 
      datasets: [{ 
        data: [0,0,0,0], 
        backgroundColor: [accent, 'rgba(251, 191, 36, 0.8)', 'rgba(245, 158, 11, 0.8)', 'rgba(255, 68, 68, 0.8)'], 
        borderColor: [accent, '#fbbf24', '#f59e0b', '#ff4444'], 
        borderWidth: 2, 
        borderRadius: 6, 
        barPercentage: 0.6 
      }] 
...
function updateSecondaryCharts(statsData, linksData){
  ...
  if(dashProtoChart && linksData){
    const protoCounts = { 'vless-ws': 0, 'xhttp-packet-up': 0, 'xhttp-stream-up': 0, 'trojan-ws': 0 };
    linksData.forEach(l => {
      const protos = l.protocols || ['vless-ws'];
      protos.forEach(p => { if(protoCounts[p] !== undefined) protoCounts[p]++; });
    });
    const counts = [protoCounts['vless-ws'], protoCounts['xhttp-packet-up'], protoCounts['xhttp-stream-up'], protoCounts['trojan-ws']];
...
async function createLink(){
  ...
  const protocols = getSelectedProtocolsV();
  ...
  try{
    ...
    document.querySelectorAll('.proto-btn-v').forEach(btn => btn.classList.remove('active'));
    document.querySelector('.proto-btn-v[data-proto="vless-ws"]').classList.add('active');
...
async function loadConns(){
  try{
    ...
    grid.innerHTML = conns.map(c => {
      ...
      // برای تروجان، transport مقدار 'trojan-ws' است
...
# توابع صفحه‌های عمومی (با Trojan)
