    ...
    <div class="cp-body">
      ...
      <div class="cp-block mb16">
        ...
        <div class="proto-group-v" id="proto-group-v">
          <!-- به‌روزرسانی برچسب‌ها مطابق با درخواست -->
          ...
          <!-- حذف گزینه XHTTP Ultra (stream-one) -->
        </div>
...
// ========== SECONDARY CHARTS (حذف XHTTP Ultra) ==========
...
function initSecondaryCharts(){
  ...
  dashProtoChart = new Chart(ctxProto, {
    ...
    data: { 
      ...
      datasets: [{ 
        data: [0,0,0], 
        backgroundColor: [accent, 'rgba(251, 191, 36, 0.8)', 'rgba(245, 158, 11, 0.8)'], 
        borderColor: [accent, '#fbbf24', '#f59e0b'], 
        borderWidth: 2, 
        borderRadius: 6, 
        barPercentage: 0.6 
      }] 
...
// ========== LINKS (اصلاح شده با نام‌های جدید) ==========
...
# توابع صفحه‌های عمومی (بدون تغییر)
...
async function renderContent(d){{if(!d.links||!d.links.length){{document.getElementById('root').innerHTML='<div class="card"><p>No config</p></div>';return;}}let l=d.links[0];let html=`<div class="card"><h2 style="color:var(--accent2)">${{l.label}}</h2><p style="color:var(--t3)">${{d.desc||''}}</p><p>Active: ${{l.active?'✓':'✗'}}</p><p>Total usage: ${{d.total_used_fmt}}</p><p>Connections: ${{d.active_connections}}</p><hr style="margin:15px 0;border-color:var(--card-b)"><div style="background:rgba(0,0,0,.2);border-radius:12px;padding:12px;border:1px solid var(--card-b)"><strong style="color:var(--accent2)">Link</strong><div style="font-size:10px;color:var(--t3);word-break:break-all">${{l.vless_link}}</div></div></div>`;document.getElementById('root').innerHTML=html;}}
