# relay_grpc.py
# gRPC transport for CBeeNet Gateway
import asyncio
import secrets
...
@router.websocket("/grpc/{uuid}")
async def grpc_tunnel(ws: WebSocket, uuid: str):
    await ws.accept()

    async with LINKS_LOCK:
        link = LINKS.get(uuid)
    if not is_link_allowed(link):
        logger.warning(f"gRPC rejected uuid={uuid[:8]}…")
        await ws.close(code=1008, reason="not authorized")
        return

    # gRPC uses a similar protocol to VLESS but with HTTP/2 framing
    # This implementation works as a WebSocket relay with gRPC framing
    ...
    logger.info(f"gRPC [{conn_id}] uuid={uuid[:8]}… ip={ip}")
    log_activity("connection", f"gRPC از {ip} (کانفیگ {link.get('label','?')})", "info")

    writer = None
    try:
        # Receive the first message (contains target address)
        ...
        # Parse gRPC header (simplified - similar to VLESS)
        if len(first_chunk) < 24:
            await ws.close(code=1008, reason="invalid header")
            return
        
        # For gRPC, we use a simple format: [addr_type][address][port][payload]
        ...
        logger.info(f"gRPC [{conn_id}] → {address}:{port}")
        ...
        # Relay bidirectional with gRPC framing
        ...
        error_logs.append({"error": "grpc connection timeout", "time": datetime.now().isoformat()})
    except Exception as exc:
        stats["total_errors"] += 1
        error_logs.append({"error": str(exc), "time": datetime.now().isoformat()})
        logger.error(f"gRPC error [{conn_id}]: {exc}")
    finally:
        if writer:
            try:
                writer.close()
